...
$cateringtype= mysqli_real_escape_string($this->connection,$data->cateringtype);
 ...
 $roombookingmodelobj=new roombookingmodelclass();
  ...
  $roombookingform1=$roombookingmodelobj->insertform1($customername,$gender,$customeremail ,$customerage, $customerphoneno);
$lastid=$roombookingform1;

$roombookingform2=$roombookingmodelobj->insertform2($purpose,$idtype, $idno, $roomtype,$roomsize,$checkindate, $checkintime,$catering,$checkoutdate, $checkouttime,$laundry,$cateringtype, $laundrytype,$lastid);
if($roombookingform1 && $roombookingform2) 
{  
...
public function searchfilteredrecord($data)
{
  $roombookingmodelobj=new roombookingmodelclass();
  $searchfilterrecord=mysqli_real_escape_string($this->connection,$data->searchrecord);  
  $filtereddata=$roombookingmodelobj->searchfilteredrecord($searchfilterrecord);
  if($filtereddata)
  {
    echo "filtered record";
  }
  else
  {
    echo "Not filtered record";
  }
}
...
if(isset($_GET['action'])){
  ...
  else if($_GET['action'] =="searchfilteredrecord")
  {
 
    $controller->searchfilteredrecord($data);
  }
}
